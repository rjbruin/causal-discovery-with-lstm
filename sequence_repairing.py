...
def get_batch(isTrain, dataset, model, debug=False):    
    ...
    while (len(batch) < model.minibatch_size):
        ...
        expression, expression_prime, _, _ = storage.get_random();
        batch.append((expression, expression_prime));
        nrSamples += 1;
    ...
    for (expression, expression_prime) in batch:
        data, targets, labels, expressions, _ = dataset.processor(";".join([expression, expression_prime]), data, targets, labels, expressions);
    ...
    while k < total:
        ...
        for j in range(nrSamples):
            ...
            if (returnTestSamples):
                ...
                strPrediction = map(lambda x: dataset.findSymbol[x], predictions[j]);
